...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    {
        $collection = new Container('MUPostsModule');
        ...
        if (count($amounts) > 0) {
            foreach ($amounts as $amountInfo) {
                ...
                $route = 'mupostsmodule_' . strtolower($amountInfo['objectType']) . '_adminview';
                $routeArgs = [
     ...
     * MUPostsModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/muposts';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/MU/PostsModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/MU/PostsModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/muposts/js/MUPostsModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mupostsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/muposts/scribite/CKEditor/mupostsmodule/',
            'file' => 'plugin.js',
            'img' => 'ed_mupostsmodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mupostsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/muposts/scribite/Quill/mupostsmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mupostsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/muposts/scribite/Summernote/mupostsmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'mupostsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/muposts/scribite/TinyMce/mupostsmodule/plugin.js'
        ]);
    }
}
