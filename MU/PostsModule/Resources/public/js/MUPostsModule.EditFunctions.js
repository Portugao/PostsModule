 ...
 */
function mUPostsInitDateField(fieldName) {
...
function mUPostsTriggerFormValidation() {
    mUPostsExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function mUPostsHandleFormSubmit (event) {
    if (triggerValidation) {
        mUPostsTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
            event.preventDefault();
            return false;
        }
 ...
 */
function mUPostsInitEditForm(mode, entityId) {
    if (jQuery('.muposts-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.muposts-edit-form').first();
    ...
    editForm.submit(mUPostsHandleFormSubmit);
