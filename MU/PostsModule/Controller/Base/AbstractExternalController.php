...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        $component = 'MUPostsModule:' . ucfirst($objectType) . ':';
        if (!$this->hasPermission($component, $id . '::', ACCESS_READ)) {
            return '';
        }
        
        $entityFactory = $this->get('mu_posts_module.entity_factory');
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('mu_posts_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@MUPostsModule/External/' . ucfirst($objectType) . '/' . $template, $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@MUPostsModule:css/style.css'));
        $cssAssetBag->add([$assetHelper->resolve('@MUPostsModule:css/custom.css') => 120]);
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('mupostsmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->hasPermission('MUPostsModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('MU\PostsModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if ($searchTerm != '') {
            $qb = $this->get('mu_posts_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        $query = $repository->getQueryFromBuilder($qb);
        
        list($entities, $objectCount) = $repository->retrieveCollectionResult($query, true);
        
        if (in_array($objectType, ['content'])) {
            $featureActivationHelper = $this->get('mu_posts_module.feature_activation_helper');
            if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
                $entities = $this->get('mu_posts_module.category_helper')->filterEntitiesByPermission($entities);
            }
        }
        ...
        $output = $this->renderView('@MUPostsModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
