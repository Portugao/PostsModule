...
namespace MU\PostsModule\Form\Handler\Content\Base;

use MU\PostsModule\Form\Handler\Common\EditHandler;
use MU\PostsModule\Form\Type\ContentType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        $this->objectTypeCapital = 'Content';
        $this->objectTypeLower = 'content';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the content yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'MUPostsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    {
        return $this->formFactory->create(ContentType::class, $this->entityRef, $this->getFormOptions());
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of content area
        $codes[] = 'userIndex';
        // admin index page of content area
        $codes[] = 'adminIndex';
        // user list of contents
        $codes[] = 'userView';
        // admin list of contents
        $codes[] = 'adminView';
        // user list of own contents
        $codes[] = 'userOwnView';
        // admin list of own contents
        $codes[] = 'adminOwnView';
        // user detail page of treated content
        $codes[] = 'userDisplay';
        // admin detail page of treated content
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl(array $args = [])
    {
        ...
        if (null !== $this->returnTo) {
            ...
            $isDisplayOrEditPage = $refererParts[count($refererParts)-2] == 'content';
        ...
        $routePrefix = 'mupostsmodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Content created.');
                } else {
                    $message = $this->__('Done! Content updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Content deleted.');
                break;
            default:
                $message = $this->__('Done! Content updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUPostsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'content', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl(array $args = [])
    {
        ...
        if ($this->request->getSession()->has('mupostsmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('mupostsmodule' . $this->objectTypeCapital . 'Referer');
        }
