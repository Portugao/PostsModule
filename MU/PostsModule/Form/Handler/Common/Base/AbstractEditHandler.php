...
namespace MU\PostsModule\Form\Handler\Common\Base;
...
use MU\PostsModule\Helper\HookHelper;
use MU\PostsModule\Helper\ModelHelper;
...
abstract class AbstractEditHandler
{
     ...
     */
    protected $entitiesWithUniqueSlugs = ['content'];
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        // default to referer
        $refererSessionVar = 'mupostsmodule' . $this->objectTypeCapital . 'Referer';
        ...
        $this->permissionComponent = 'MUPostsModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUPostsModule' . $this->objectTypeCapital . $entity->getKey();
                    $this->lockingApi->addLock($lockName, $this->getRedirectUrl(['commandName' => '']));
                }
            }
        } else {
            $permissionLevel = in_array($this->objectType, ['content']) ? ACCESS_COMMENT : ACCESS_EDIT;
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUPostsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
    ...
    public function handleCommand(array $args = [])
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            if ($action != 'delete') {
                ...
                $routeUrl = new RouteUrl('mupostsmodule_' . $this->objectTypeLower . '_display', $urlArgs);
            }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'MUPostsModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('MUPostsModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('MUPostsModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('MUPostsModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];
