...
namespace MU\PostsModule\Menu\Base;
...
class AbstractItemActionsMenu implements ContainerAwareInterface
{
    ...
    public function menu(FactoryInterface $factory, array $options = [])
    {
        ...
        $entityDisplayHelper = $this->container->get('mu_posts_module.entity_display_helper');
        ...
        if ($entity instanceof ContentEntity) {
            $component = 'MUPostsModule:Content:';
            $instance = $entity->getKey() . '::';
            $routePrefix = 'mupostsmodule_content_';
        
            if ($routeArea == 'admin') {
                $title = $this->__('Preview', 'mupostsmodule');
                $menu->addChild($title, [
                    'route' => $routePrefix . 'display',
                    'routeParameters' => $entity->createUrlArgs()
                ]);
                $menu[$title]->setLinkAttribute('target', '_blank');
                $menu[$title]->setLinkAttribute('title', $this->__('Open preview page', 'mupostsmodule'));
                $menu[$title]->setAttribute('icon', 'fa fa-search-plus');
            }
            if ($context != 'display') {
                $title = $this->__('Details', 'mupostsmodule');
                $menu->addChild($title, [
            ...
            if ($permissionApi->hasPermission($component, $instance, ACCESS_EDIT)) {
                $title = $this->__('Edit', 'mupostsmodule');
                $menu->addChild($title, [
                    'route' => $routePrefix . $routeArea . 'edit',
                    'routeParameters' => $entity->createUrlArgs()
                ]);
                $menu[$title]->setLinkAttribute('title', $this->__('Edit this content', 'mupostsmodule'));
                $menu[$title]->setAttribute('icon', 'fa fa-pencil-square-o');
                $title = $this->__('Reuse', 'mupostsmodule');
                $menu->addChild($title, [
                    'route' => $routePrefix . $routeArea . 'edit',
                    'routeParameters' => ['astemplate' => $entity->getKey()]
                ]);
                $menu[$title]->setLinkAttribute('title', $this->__('Reuse for new content', 'mupostsmodule'));
                $menu[$title]->setAttribute('icon', 'fa fa-files-o');
            }
            if ($permissionApi->hasPermission($component, $instance, ACCESS_DELETE)) {
                $title = $this->__('Delete', 'mupostsmodule');
                $menu->addChild($title, [
                    'route' => $routePrefix . $routeArea . 'delete',
                    'routeParameters' => $entity->createUrlArgs()
                ]);
                $menu[$title]->setLinkAttribute('title', $this->__('Delete this content', 'mupostsmodule'));
