...
abstract class AbstractWorkflowHelper
{
    ...
    public function executeAction(EntityAccess $entity, $actionId = '', $recursive = false)
    {
        ...
        $logArgs = ['app' => 'MUPostsModule', 'user' => $this->currentUserApi->get('uname')];
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'content';
        if ($this->permissionApi->hasPermission('MUPostsModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'contentsApproval',
                    'description' => $this->translator->__('Contents pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One content is waiting for approval.', '%amount% contents are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUPostsModule', 'amount' => $amount, 'entities' => 'contents']);
            }
        }
