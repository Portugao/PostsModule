...
use MU\PostsModule\Helper\ArchiveHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in MUPostsModule.
     *
    ...
    public function getObjectTypes($context = '', array $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'content';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in MUPostsModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('MUPostsModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('MUPostsModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('MU\PostsModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('mupostsmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('mupostsmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
