...
abstract class AbstractSearchHelper implements SearchableInterface
{
    ...
    {
        if (!$this->permissionApi->hasPermission('MUPostsModule::', '::', ACCESS_READ)) {
            return '';
        }
    ...
    public function getResults(array $words, $searchType = 'AND', $modVars = null)
    {
        ...
        $searchTypes = $this->getSearchTypes();
        $entitiesWithDisplayAction = ['content'];
        ...
        foreach ($searchTypes as $searchTypeCode => $typeInfo) {
            ...
            if (isset($moduleActivationInfo['MUPostsModule'])) {
                $moduleActivationInfo = $moduleActivationInfo['MUPostsModule'];
            ...
            switch ($objectType) {
                case 'content':
                    ...
                    $whereArray[] = 'tbl.target';
                    $whereArray[] = 'tbl.startText';
                    $whereArray[] = 'tbl.firstMainText';
                    $whereArray[] = 'tbl.secondMainText';
                    $whereArray[] = 'tbl.experienceOfNature';
                    $whereArray[] = 'tbl.levelOfDifficulty';
                    $whereArray[] = 'tbl.requiredFitness';
                    $whereArray[] = 'tbl.other';
                    break;
            }
            ...
            foreach ($entities as $entity) {
                ...
                if (!$this->permissionApi->hasPermission('MUPostsModule:' . ucfirst($objectType) . ':', $entity->getKey() . '::', ACCESS_OVERVIEW)) {
                    continue;
                }
                ...
                if ($hasDisplayAction) {
                    ...
                    $displayUrl = new RouteUrl('mupostsmodule_' . strtolower($objectType) . '_display', $urlArgs);
                }
    
                $result = new SearchResultEntity();
                $result->setTitle($formattedTitle)
                    ->setText($description)
                    ->setModule('MUPostsModule')
                    ->setCreated($created)
