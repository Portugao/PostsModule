...
abstract class AbstractArchiveHelper
{
    ...
    public function archiveObsoleteObjects($probabilityPercent = 75)
    {
        ...
        if (!$this->permissionApi->hasPermission('MUPostsModule', '.*', ACCESS_EDIT)) {
            // abort if current user has no permission for executing the archive workflow action
            return;
        }
    
        // perform update for contents becoming archived
        $logArgs = ['app' => 'MUPostsModule', 'entity' => 'content'];
    ...
    /**
     * Moves contents into the archive which reached their end date.
     *
     * @throws RuntimeException Thrown if workflow action execution fails
     */
    protected function archiveContents()
    {
        $today = date('Y-m-d H:i:s');
    
        $affectedEntities = $this->getObjectsToBeArchived('content', 'endDate', $today);
    ...
    protected function archiveSingleObject($entity)
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            $hasDisplayPage = in_array($objectType, ['content']);
            if ($hasDisplayPage) {
                $urlArgs = $entity->createUrlArgs();
                if (null !== $request) {
                    $urlArgs['_locale'] = $request->getLocale();
                }
                $url = new RouteUrl('mupostsmodule_' . strtolower($objectType) . '_display', $urlArgs);
        	}
