...
abstract class AbstractNotificationHelper
{
    ...
    protected function collectRecipients($debug = false)
    {
        ...
        if ($this->recipientType == 'moderator' || $this->recipientType == 'superModerator') {
            ...
            $moderatorGroupId = $this->variableApi->get('MUPostsModule', 'moderationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            if ($this->recipientType == 'superModerator') {
                $moderatorGroupId = $this->variableApi->get('MUPostsModule', 'superModerationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            }
    ...
    protected function sendMails()
    {
        ...
        foreach ($this->recipients as $recipient) {
            ...
            $body = $this->templating->render('@MUPostsModule/' . $template, [
                'recipient' => $recipient,
                'mailData' => $mailData
            ]);
            $altBody = '';
    ...
    protected function prepareEmailData()
    {
        ...
        $hasDisplayAction = in_array($objectType, ['content']);
        $hasEditAction = in_array($objectType, ['content']);
        $routeArea = in_array($this->recipientType, ['moderator', 'superModerator']) ? 'admin' : '';
        $routePrefix = 'mupostsmodule_' . strtolower($objectType) . '_' . $routeArea;
